...
public class KelompokStudiMultimediaController {
    ...
    public String report(@RequestParam("format") String format, @RequestParam(value = "status", required = false, defaultValue = "true") Boolean status, ModelMap modelMap) {
        logger.debug("recieved request to get report " + format + " ksm " + status);

        modelMap.addAttribute("datasource", kelompokStudiMultimediaService.findAll());
        modelMap.addAttribute("show", status);

        if (format.equalsIgnoreCase("pdf")) {
            return "studyPdfReport";
        } else if (format.equalsIgnoreCase("xls")) {
            return "studyXlsReport";
        } else if (format.equalsIgnoreCase("csv")) {
            return "studyCsvReport";
        } else {
            return "studyHtmlReport";
        }
    }
}
